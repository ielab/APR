...
from pyserini.dsearch import SimpleDenseSearcher, BinaryDenseSearcher, TctColBertQueryEncoder, QueryEncoder, \
    ...
    DenseVectorRocchioPrf, DenseVectorAncePrf, DenseVectorDistillBERTPrf, DenseVectorTCTColBERTV2HNPPrf
    ...
    with output_writer:
        ...
        for index, (topic_id, text) in enumerate(tqdm(query_iterator, total=len(topics.keys()))):
            if args.batch_size <= 1 and args.threads <= 1:
                if PRF_FLAG:
                    emb_q, prf_candidates = searcher.search(text, k=args.prf_depth, return_vector=False, **kwargs)
                    # ANCE-PRF input is different, do not need query embeddings
                    if args.prf_method.lower() == 'ance-prf' or args.prf_method.lower() == 'distillbert-prf' or args.prf_method.lower() == 'tctv2-prf':
