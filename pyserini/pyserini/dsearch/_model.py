...
class AncePrfEncoder(torch.nn.Module):
    config_class = RobertaConfig
    base_model_prefix = 'ance_encoder'
    load_tf_weights = None
    _keys_to_ignore_on_load_missing = [r'position_ids']
    ...
    def __init__(self, path):
        super().__init__()
        requires_backends(self, 'torch')
        self.roberta = RobertaModel.from_pretrained(path).eval()
        self.embeddingHead = torch.nn.Linear(768, 768).eval()
        self.norm = torch.nn.LayerNorm(768)
        import os
        model_dict = torch.load(os.path.join(path, 'pooler.pt'), map_location="cpu")
