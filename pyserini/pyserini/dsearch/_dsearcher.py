...
from ._model import AnceEncoder, AncePrfEncoder
import torch
...
class AncePrfQueryEncoder:

    def __init__(self, encoder_dir: str = None, tokenizer_name: str = None, device: str = 'cpu'):
        if encoder_dir:
            self.device = device
            self.model = AncePrfEncoder(encoder_dir)
            self.model.to(self.device)
            self.tokenizer = RobertaTokenizer.from_pretrained(tokenizer_name or encoder_dir)
            self.tokenizer.do_lower_case = True
